...
//    auto lShader = ShaderManager::getDefaultShader();
    ...
    auto lShader1 =
        ShaderManager::getShader(QList<QString>({lPath + QString("shader/phong_bump_fl.vert"), lPath + QString("shader/phong_bump_fl.frag")}));
    auto lMeshOBJ = MeshOBJ(lPath+QString("models/skyship.obj"));
    lMeshOBJ.setGenerateFaceNormals(true);

    auto lEnt = addRenderable<GeometryIndexedBase, MeshOBJ>(lMeshOBJ, lShader1);
...
//    lTrans.translate(5.,0.,0.);
    ...
    auto lEntOther = addRenderable<GeometryBase, SimplePlane>(SimplePlane(10.0f), lShader);
