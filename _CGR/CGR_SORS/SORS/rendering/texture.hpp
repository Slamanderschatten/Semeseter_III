...
public:
    ...
    void setAnisotropyLevel(unsigned int level) {mAnisotropyLevel = std::min((unsigned int)mMaximumAnisotropy, level);}
    void setNameString(QString pName) {mTexNameString=pName;}
    virtual void makeStripes(int size, int thickness);
    void makeDummy();
    virtual ~Texture();
    ...
    bool getBuildMipmaps() const;
    void setBuildMipmaps(bool pBuildMipmaps);
...
protected:
    ...
    int mMaxFilter;
    int mWrapMode;
    bool mBuildMipmaps{true};
    float mMaximumAnisotropy;
    unsigned int mAnisotropyLevel{1};
    bool mInitialized;
