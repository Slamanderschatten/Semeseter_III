...
bool Texture::getBuildMipmaps() const
{
    return mBuildMipmaps;
}

void Texture::setBuildMipmaps(bool pBuildMipmaps)
{
    mBuildMipmaps = pBuildMipmaps;
}
...
void Texture::textureInit()
{
    ...
    if (!mInitialized)
    {
        ...
        glFunctions->glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, mAnisotropyLevel);
        this->loadImageToGPU();
...
void Texture::makeStripes(int size, int thickness)
{
    int i, j;
    bool isWhite = true;
