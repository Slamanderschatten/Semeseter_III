...
void Manager::initialize()
{
    ...
    auto lShaderWaving = ShaderManager::getShader(
        QList<QString>({lPath + QString("shader/rotate.vert"), lPath + QString("shader/hello_glsl.frag")}));
    ...
    lEnt1 = addRenderable<GeometryBase, SimplePlane>(SimplePlane(3.0f, 3.0f, 6), lShaderWaving);
    ECS.get<Transformation>(lEnt1).translate(3.0f, .0f,.0f);
    ECS.get<Renderable>(lEnt1).addProperty(std::make_shared<TimerUniform>());
}
